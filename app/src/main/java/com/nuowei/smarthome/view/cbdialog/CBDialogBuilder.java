...
import com.nuowei.smarthome.view.cbdialog.pedant.SweetAlert.ProgressHelper;
import com.nuowei.smarthome.view.cbdialog.pnikosis.materialishprogress.ProgressWheel;
import com.nuowei.smarthome.view.cbdialog.titanic.Titanic;
import com.nuowei.smarthome.view.cbdialog.titanic.TitanicTextView;
...
public class CBDialogBuilder {
    ...
    /**
     * @param duration                the result = 800*7*duration,duration default 1000
    ...
    public CBDialogBuilder setItems(final List<XlinkDevice> xlinkDeviceList,
                                    final onDialogItemXlinkClickListener listener, final String devicemac) {
        if (DIALOG_STYLE_CURRENT != DIALOG_STYLE_NORMAL) {
            return this;
        }
        this.showIcon(false);
        ...
        // ArrayAdapter<String> adapter = new ArrayAdapter<String>(context,
        // R.layout.item_option_text, items);
        final DialogItemXlinkAdapter adapter = new DialogItemXlinkAdapter(xlinkDeviceList,
                devicemac);
        listview.setAdapter(adapter);
        listview.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view,
                ...
                // let the developer decision whether or not dismiss
                // dialog.dismiss();
                adapter.setSelectedPos(xlinkDeviceList.get(position).getDeviceMac());
                adapter.notifyDataSetChanged();
                if (listener != null) {
                    listener.onDialogItemClick(adapter, context,
                            CBDialogBuilder.this, dialog, position);
     ...
     *
     * @param indicator {@link #INDICATOR_BallPulse}
    ...
    public interface onDialogItemXlinkClickListener {


        public void onDialogItemClick(DialogItemXlinkAdapter adapter, Context context, CBDialogBuilder cbDialogBuilder, Dialog dialog, int position);
     ...
     * @author long
     */
    public class DialogItemXlinkAdapter extends BaseAdapter {
        private List<XlinkDevice> xlinkDeviceList;
        private String devicemac;

        public DialogItemXlinkAdapter(List<XlinkDevice> xlinkDeviceList, String devicemac) {
            this.xlinkDeviceList = xlinkDeviceList;
            this.devicemac = devicemac;
        }

        @Override
        public int getCount() {
            return xlinkDeviceList == null ? 0 : xlinkDeviceList.size();
        }

        @Override
        public Object getItem(int position) {
            return xlinkDeviceList.get(position);
        }
        ...
        public void setSelectedPos(String devicemac) {
            this.devicemac = devicemac;
        }
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (devicemac.equals(xlinkDeviceList.get(position).getDeviceMac())) {
                viewHolder.txView.setTextColor(context.getResources().getColor(
                        R.color.item_text_color_pressed));
                viewHolder.txView
                        .setBackgroundResource(R.drawable.item_tx_background);
            ...
            viewHolder.txView.setText(xlinkDeviceList.get(position).getDeviceName());
            return convertView;
        }
