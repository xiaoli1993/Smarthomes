	...
	public void getIsConnectApWifi(){
		...
		if(wifiName.startsWith(AppConfig.Relese.APTAG)){
		    String contactId=wifiName.substring(length,wifiName.length());
		 	Account	account =AccountPersist.getInstance().getActiveAccountInfo(MyApplication.app);
			if(account==null){
				account=new Account();
			}
			...
			account.rCode2 = "0";
			account.sessionId = "0";
			AccountPersist.getInstance().setActiveAccount(MyApplication.app, account);
			NpcCommon.mThreeNum=AccountPersist.getInstance()
					.getActiveAccountInfo(MyApplication.app).three_number;
...
//			 gainIsApMode(contactId);
	...
	public void gainIsApMode(String contactId){
 		...
 		try {
 			tcpClient.setIpdreess(InetAddress.getByName(Utils.getAPDeviceIp(context)));
			...
			case TcpClient.SEARCH_AP_DEVICE:
				...
				if(wifiName.startsWith(AppConfig.Relese.APTAG)){
					...
					if(id.equals(contactId)){
						...
						try {
							apdevice.address = InetAddress.getByName(Utils.getAPDeviceIp(MyApplication.app));
						} catch (UnknownHostException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
