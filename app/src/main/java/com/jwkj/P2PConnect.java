...
public class P2PConnect {
	...
	public static synchronized void vReject(int resoncode,String msg) {
		...
		try {
			...
			MyApplication.app.sendBroadcast(refreshContans);
...
//			return;
//		}
//		AlarmRecord alarmRecord = new AlarmRecord();s
...
//		}
	...
	public static synchronized void vAllarmingWithPath(String id, int type,
		...
		Log.e("leleTest","mainType="+mainType);
		...
		if(Utils.is868Device(mainType, subType)){
			if(((option>>4)&0x1)==1){
				hasPictrue=true;
			}else{
				hasPictrue=false;
...
//		}
		...
		if (!isPlaying) {
	    		...
	    		MyApplication.app.startActivity(alarm);
		} else {// 正在监控
				Intent k = new Intent();
				k.setAction(Constants.Action.MONITOR_NEWDEVICEALARMING);
				k.putExtra("messagetype", 1);
