...
public class DataManager {
	...
	public static synchronized List<AlarmRecord> findAlarmRecordByActiveUser(
			Context context, String activeUserId,int[] num) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			...
			List<AlarmRecord> lists = alarmRecordDB
					.findByActiveUserId(activeUserId,num);
			db.close();
			return lists;
		}
	}
    ...
    //查找未读的报警记录  (1.context 2.当前登录的用户 3.条数范围)
    public static synchronized List<AlarmRecord> findNoCheckByActiveUserId(
            Context context, String activeUserId, int limit){
        synchronized (DataManager.class) {
            DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
                    DataBaseVersion);
            SQLiteDatabase db = dbHelper.getWritableDatabase();
            AlarmRecordDB alarmDB = new AlarmRecordDB(db);
            List<AlarmRecord> lists = alarmDB
                    .findNoCheckByActiveUserId(activeUserId,limit);
            db.close();
            return lists;
        }
    }
    ...
    //查询系统消息
      ...
      //添加系统消息   
		 ...
		 * @param point
		 */
		public static synchronized void insertDefenceArea(Context context,String deviceId,DefenceArea defenceArea) {
			synchronized (DataManager.class) {
				DBHelper dbHelper = new DBHelper(context, DataBaseName, null, DataBaseVersion);
				SQLiteDatabase db = dbHelper.getWritableDatabase();
				DefenceAreaDB defenceAreaDB = new DefenceAreaDB(db);
				defenceAreaDB.insert(NpcCommon.mThreeNum,deviceId,defenceArea);
				db.close();
			}
		}
		 ...
		 * @param point
		 */
		public static synchronized List<DefenceArea> findAllDefenceAreaByDeviceID(Context context,String deviceId) {
			synchronized (DataManager.class) {
				DBHelper dbHelper = new DBHelper(context, DataBaseName, null, DataBaseVersion);
				SQLiteDatabase db = dbHelper.getWritableDatabase();
				DefenceAreaDB defenceAreaDB = new DefenceAreaDB(db);
				List<DefenceArea> defenceAreas=new ArrayList<DefenceArea>();
				defenceAreas=defenceAreaDB.findAllDefenceAreaByDeviceID(NpcCommon.mThreeNum, deviceId);
				db.close();
				return defenceAreas;
			}
		}
		 ...
		 * @param point
		 */
		public static synchronized DefenceArea findDefenceAreaByDeviceID(Context context,String deviceId,DefenceArea area) {
			synchronized (DataManager.class) {
				DBHelper dbHelper = new DBHelper(context, DataBaseName, null, DataBaseVersion);
				SQLiteDatabase db = dbHelper.getWritableDatabase();
				DefenceAreaDB defenceAreaDB = new DefenceAreaDB(db);
				DefenceArea defenceArea=defenceAreaDB.findDefenceAreaByDeviceID(NpcCommon.mThreeNum, deviceId,area);
				db.close();
				return defenceArea;
			}
		}
		 ...
		 * @param point
		 */
		public static synchronized long updateDefenceAreaByGroupAndItem(Context context,String deviceId,DefenceArea defenceArea) {
			synchronized (DataManager.class) {
				DBHelper dbHelper = new DBHelper(context, DataBaseName, null, DataBaseVersion);
				SQLiteDatabase db = dbHelper.getWritableDatabase();
				DefenceAreaDB defenceAreaDB = new DefenceAreaDB(db);
				long result=defenceAreaDB.updateDefenceAreaByGroupAndItem(NpcCommon.mThreeNum, deviceId,defenceArea);
				db.close();
				return result;
			}
		}
		 ...
		 * @param point
		 */
		public static synchronized long updateDefenceAreaNameByGroupAndItem(Context context,String deviceId,String name,DefenceArea defenceArea) {
			synchronized (DataManager.class) {
				DBHelper dbHelper = new DBHelper(context, DataBaseName, null, DataBaseVersion);
				SQLiteDatabase db = dbHelper.getWritableDatabase();
				DefenceAreaDB defenceAreaDB = new DefenceAreaDB(db);
				long result=defenceAreaDB.updateDefenceAreaNameByGroupAndItem(NpcCommon.mThreeNum, deviceId,name,defenceArea);
				db.close();
				return result;
			}
		}
		 ...
		 * @param point
		 */
		public static synchronized long updateDefenceAreaNameAndEflagByGroupAndItem(Context context,String deviceId,String name,int eflag,int group, int item) {
			synchronized (DataManager.class) {
				DBHelper dbHelper = new DBHelper(context, DataBaseName, null, DataBaseVersion);
				SQLiteDatabase db = dbHelper.getWritableDatabase();
				DefenceAreaDB defenceAreaDB = new DefenceAreaDB(db);
				long result=defenceAreaDB.updateDefenceAreaNameAndEflagByGroupAndItem(NpcCommon.mThreeNum, deviceId,name,eflag,group,item);
				db.close();
				return result;
			}
		}
		 ...
		 * @param point
		 */
		public static synchronized int deleteAllDefenceAreaByDeviceID(Context context,String deviceId) {
			synchronized (DataManager.class) {
				DBHelper dbHelper = new DBHelper(context, DataBaseName, null, DataBaseVersion);
				SQLiteDatabase db = dbHelper.getWritableDatabase();
				DefenceAreaDB defenceAreaDB = new DefenceAreaDB(db);
				int result=defenceAreaDB.deleteAllDefenceAreaByDeviceID(NpcCommon.mThreeNum, deviceId);
				db.close();
				return result;
			}
		}
		 ...
		 * @param point
		 */
		public static synchronized int deleteDefenceAreaByGroupAndItem(Context context,String deviceId,DefenceArea defenceArea) {
			synchronized (DataManager.class) {
				DBHelper dbHelper = new DBHelper(context, DataBaseName, null, DataBaseVersion);
				SQLiteDatabase db = dbHelper.getWritableDatabase();
				DefenceAreaDB defenceAreaDB = new DefenceAreaDB(db);
				int result=defenceAreaDB.deleteDefenceAreaByGroupAndItem(NpcCommon.mThreeNum, deviceId,defenceArea);
				db.close();
				return result;
			}
		}
