...
public class PlayBackListActivity extends BaseActivity implements
	...
	ImageView back;
	TextView search_detail, search_one_day, search_three_day, search_all;
	ImageView cursor;
	Contact contact;
	...
	public void initComponent() {
		...
		search_all = (TextView) findViewById(R.id.search_all);
	...
	BroadcastReceiver receiver = new BroadcastReceiver() {
		...
		public void onReceive(Context arg0, Intent intent) {
			if (intent.getAction().equals(Constants.P2P.RET_GET_PLAYBACK_FILES)) {
				...
				byte option1=intent.getByteExtra("option1", (byte) -1);
				if(option1!=-1){
					if(option1==Constants.P2P_SET.SD_FORMAT.SD_NO_EXIST){
						if(noTFcardFragment==null){
							noTFcardFragment = new NoTFcardFragment();
						}
						replaceFrag( noTFcardFragment , fragments[3]);
						return;
					}
				}
				if (isRetutn()) {
...
//					rlFrag.setList(list);
	...
	public void onClick(View arg0) {
		...
		if (i == R.id.back_btn) {
			...
			try {
				start = sdf.parse(startTime.getText().toString());
				end = sdf.parse(endTime.getText().toString());
				if (start.after(end)) {
					T.showShort(mContext, R.string.search_error3);
					return;
				}
				madapter.setStartTime(start);
				P2PHandler.getInstance().getRecordFiles(contact.getIpContactId(),
		...
		} else if (i == R.id.search_one_day) {
			searchPlaybackFile(0);

		} else if (i == R.id.search_three_day) {
			searchPlaybackFile(1);

		} else if (i == R.id.search_all) {
...
//		}
	...
	public void searchAllRecord() {
		...
		try {
			Date endDate = sdf.parse("2035-12-30 23:59");
			RecordAdapter.setStartTime(startDate);
			P2PHandler.getInstance().getRecordFiles(contact.getIpContactId(),
					contact.contactPassword, startDate, endDate);
		} catch (ParseException e) {
	...
	public void updateCondition() {
		...
		case 1:
			...
			break;
		case 2:
			search_all.setTextColor(Color.BLACK);
			search_three_day
					.setTextColor(getResources().getColor(R.color.gray));
			search_one_day.setTextColor(getResources().getColor(R.color.gray));
			search_detail.setTextColor(getResources().getColor(R.color.gray));
	...
	InputPasswordDialog.InputPasswordClickListener  inputPwdClickListener=new InputPasswordDialog.InputPasswordClickListener() {
		...
		public void onOkClick(String contactId, String pwd) {
			...
			if(c!=null){
               ...
               //重新刷新录像文件
				searchPlaybackFile(currnetTime);
			}
			if(inputPwdDialog!=null){
				inputPwdDialog.dismiss();
				inputPwdDialog=null;
	...
	public void searchPlaybackFile(int select){
		switch (select){
			case 0:
				currnetTime = 0;
				isLoadingChange = true;
