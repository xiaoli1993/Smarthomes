...
//			tv_weak_password.setVisibility(TextView.GONE);
		...
		new_pwd.addTextChangedListener(new SearchWather(new_pwd));
		re_new_pwd.addTextChangedListener(new SearchWather(re_new_pwd));
		...
		public void onReceive(Context arg0, Intent intent) {
			if (intent.getAction()
				...
				if (result == Constants.P2P_SET.DEVICE_PASSWORD_SET.SETTING_SUCCESS) {
					...
					//修改成功  改变SecurityControlFrag中的contact
					Intent modify_success = new Intent();
					modify_success.setAction(Constants.Action.MODIFY_PASSWORD_SUCCESS);
					modify_success.putExtra("contact", mContact);
					mContext.sendBroadcast(modify_success);
	...
	public void onClick(View view) {
		...
		if (i == R.id.back_btn) {
			...
			if (mContact.contactType == P2PValue.DeviceType.NVR) {
				P2PHandler.getInstance().setDevicePassword(mContact.contactId,
						password_old, password_new, userPassword, userPassword);
			} else {
				P2PHandler.getInstance().setDevicePassword(mContact.contactId,
						password_old, password_new, userPassword, userPassword);
			}
