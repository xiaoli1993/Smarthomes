...
public class ModifyBoundEmailActivity extends BaseActivity implements
	...
	public void initCompent() {
		...
		if (sendEmail.length() <= 0 || sendEmail.equals("0")
			...
			if (emails.length > 1 ||!Email.getInstence().isSurportThisSMTP(emailRoot)||!email_name.equals(senderEmail)
					&& isSurportSMTP) {
				mSend.setText("");
				mPassword.setText("");
		...
		// tv_manual_set.setOnComfirmClickListener(this);
		// img_manual_set.setOnComfirmClickListener(this);
	...
	private BroadcastReceiver mReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context arg0, Intent intent) {
			...
			if (intent.getAction().equals(Constants.P2P.RET_SET_ALARM_EMAIL)) {
				...
				if (contectid != null && contectid.equals(mContact.getContactId())) {
				...
				if (result == Constants.P2P_SET.ACK_RESULT.ACK_PWD_ERROR) {
					...
					P2PHandler.getInstance().getAlarmEmail(mContact.getContactId(),
	...
	public void onClick(View view) {
		...
		} else if (i == R.id.tv_manual_set) {
		} else if (i == R.id.img_manual_set) {// String emailRoot = etSmtp.getText().toString();// 后缀
			// String[] emailMessage =
			// Email.getInstence().getEmailMessage(emailRoot);
			// et_port.setText(emailMessage[2]);
			// changeSet();

		} else if (i == R.id.radio_one) {// 不加密
			encryptType = 0;

		} else if (i == R.id.radio_two) {// SSL加密
			encryptType = 1;

		} else if (i == R.id.radio_three) {// TLS加密
			encryptType = 2;

		} else if (i == R.id.manual_set) {
			changeSet();
	...
	public void saveManualSetEmail() {
		...
		try {
			...
			Log.e("sendmanual", "addressee=" + addressee + "--" + "port="
					...
					+ encryptType);
			isGetEmail = true;
			P2PHandler.getInstance().setAlarmEmailWithSMTP(mContact.getContactId(),
					mContact.getPassword(), (byte) 3, addressee, port,
...
//			myHandler.postDelayed(myRunnable, 2000);
	...
	void ClearEmail(boolean isOldDevice) {
		...
		if(isOldDevice){
			P2PHandler.getInstance().setAlarmEmailWithSMTP(mContact.getContactId(),
					mContact.getPassword(), (byte)3, "0", 0, "", "0", "", "", "",(byte)0,(byte)0,0);
			}else{
				P2PHandler.getInstance().setAlarmEmailWithSMTP(mContact.getContactId(),
						mContact.getPassword(), (byte)3, "0", 11, "11", "11", "11", "11", "11",(byte)11,(byte)0,0);
			}
	}
...
//		timeoutHandler.sendMessageDelayed(m, 5000);
	...
	private void saveEmail() {
		...
		isGetEmail = true;
		P2PHandler.getInstance().setAlarmEmailWithSMTP(mContact.getContactId(),
				mContact.getPassword(), (byte) 0, email, 0, "", "", "", "",
				"", (byte) 0, (byte) 0, 0);
//		myHandler.postDelayed(myRunnable, 2000);
//		Message m = new Message();
