...
public class FisheyeSetHandler {
    ...
    private static int MESG_GET_LED_STATE=Constants.MsgSection.MESG_GET_LED_STATE;
    private static int MESG_SET_LED_STATE=Constants.MsgSection.MESG_SET_LED_STATE;
    private static int MESG_ID_FISHEYE_SET_SENSOR_PREPOINT=Constants.MsgSection.MESG_ID_FISHEYE_SET_SENSOR_PREPOINT;
     ...
     * @param password
     */
    public void sSpecialAlarmData(String contactId, String password,byte[] specalData,int prepoint){
    ...
    public void sGetLedState(String contactId, String password){
    	   if ((MESG_GET_LED_STATE) >= (Constants.MsgSection.MESG_GET_LED_STATE)) {
    		   MESG_GET_LED_STATE = Constants.MsgSection.MESG_GET_LED_STATE - 1000;
           }
           byte[] fishdata=new byte[7];
           fishdata[0]=MESG_TYPE_WORKMODE_SETTING;
           fishdata[1]=FishSubCmd.MESG_SUBTYPE_GET_LED_STATU;
           P2PHandler.getInstance().setFishEye(Integer.parseInt(contactId),Integer.parseInt(password),MESG_GET_LED_STATE,fishdata);
           MESG_GET_LED_STATE++;
    }
    public void sSetLedState(String contactId, String password,int state){
    	  if ((MESG_SET_LED_STATE) >= (Constants.MsgSection.MESG_SET_LED_STATE)) {
    		  MESG_SET_LED_STATE = Constants.MsgSection.MESG_SET_LED_STATE - 1000;
          }
          byte[] fishdata=new byte[7];
          fishdata[0]=MESG_TYPE_WORKMODE_SETTING;
          fishdata[1]=FishSubCmd.MESG_SUBTYPE_CONTROL_LED;
          fishdata[3]=(byte) state;
          P2PHandler.getInstance().setFishEye(Integer.parseInt(contactId),Integer.parseInt(password),MESG_SET_LED_STATE,fishdata);
          MESG_SET_LED_STATE++;
    }
     ...
     * @param bMemo
     */
    public void sSetSenSorPrePoint(String contactId, String password,byte[] bSensorData,int bPresetPos,byte bMemo){
    	if ((MESG_ID_FISHEYE_SET_SENSOR_PREPOINT) >= (Constants.MsgSection.MESG_ID_FISHEYE_SET_SENSOR_PREPOINT)) {
    		MESG_ID_FISHEYE_SET_SENSOR_PREPOINT = Constants.MsgSection.MESG_ID_FISHEYE_SET_SENSOR_PREPOINT - 1000;
        }
    	byte[] fishdata=new byte[10];
    	fishdata[0]=MESG_TYPE_WORKMODE_SETTING;
    	fishdata[1]=FishSubCmd.MESG_SUBTYPE_SET_SESOR_PRESET_POS;
    	System.arraycopy(bSensorData, 0, fishdata, 4, bSensorData.length);
    	fishdata[8]=(byte)bPresetPos;
    	fishdata[9]=bMemo;
    	P2PHandler.getInstance().setFishEye(Integer.parseInt(contactId),Integer.parseInt(password), MESG_ID_FISHEYE_SET_SENSOR_PREPOINT, fishdata);
