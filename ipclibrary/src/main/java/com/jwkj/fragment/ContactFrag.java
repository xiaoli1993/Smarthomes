		...
		mAdapter.setOnSrttingListner(new onConnectListner() {
			...
			public void onNvrClick(Contact contact) {
				...
				JAContact jacontact = DataManager
						.findJAContactByActiveUserAndContactId(MyApp.app,
...
//					T.showShort(mContext, R.string.pw_incrrect);
	...
	private JAContact paserNVRInfoData(byte[] data, int deviceid) {
		...
		if (jacontact != null) {
			// 更新
			DataManager.updateJAContact(MyApp.app, jacon);
		} else {
			// 插入
			DataManager.insertJAContact(MyApp.app, jacon);
		}
		return jacon;
	}
...
//			mPullRefreshListView.onRefreshComplete();
	...
	public void onClick(View v) {
		int i = v.getId();
		if (i == R.id.button_add) {//			if (isHideAdd == true) {
//				showAdd();
//			} else {
//				hideAdd();
//			}
