...
//	                    T.showLong(mContext, "获取错误");
    ...
    private void getLampState(byte lamstate,Sensor sensor){
        if(sensor.isControlSensor()){
            FisheyeSetHandler.getInstance().sGetLampStatu(ApMonitorActivity.mContact.contactId, ApMonitorActivity.mContact.contactPassword, lamstate, sensor.getSensorData());
        }
    }
    /**
     * 通过特征码获取列表的传感器
